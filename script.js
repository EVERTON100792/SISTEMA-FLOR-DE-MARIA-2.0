// ===== SISTEMA DE GESTÃO INTEGRADO - FLOR DE MARIA v3.2 (Full Firebase Integration & UX/Responsive Fix) =====
...
    getToday: () => new Date().toISOString().split('T')[0],
    // **NOVA FUNÇÃO CENTRALIZADA** para popular seletores de cliente
    populateClientSelects: (selectIds, defaultOptionText = 'Selecione um cliente') => {
        selectIds.forEach(selectId => {
            ...
            if(select) {
                ...
                select.value = currentValue; // Tenta manter o valor selecionado anteriormente
            }
        });
    }
};
...
        state.clients = []; // Limpa o estado local ao sair
...
            });
            ...
            // **AÇÃO ADICIONADA**: Após carregar todos os dados, atualiza os seletores de cliente
            // Isso garante que a lista de clientes esteja sempre disponível para as outras partes do sistema.
...
        
        Clients.clearForm();
        await App.loadAllData(); // Recarrega todos os dados e atualiza os selects
        await Clients.load();
    },
    edit(id) {
...
    },
    async load() {
        // **AÇÃO MODIFICADA**: Usa a função utilitária para garantir que o seletor seja populado
        Utils.populateClientSelects(['saleClient'], 'Consumidor Final');
        this.clearCart(); // Limpa o carrinho ao entrar na página
    },
    showSuggestions() {
...
            const productRef = db.collection(CONFIG.collections.products).doc(item.id);
            // **AÇÃO CORRIGIDA**: A forma correta de decrementar o valor no Firestore
...
            let statusClass = 'badge-info'; // Pendente
...
        const today = new Date();
        today.setHours(0, 0, 0, 0); // Compara apenas com o início do dia
        const batch = db.batch();
...
                hasChanges = true;
                r.status = 'Vencido'; // Atualiza o estado local imediatamente
            }
        });
...
                // Não precisa recarregar tudo, o estado local já foi atualizado
            } catch (error) {
                ...
                for (const collectionName of collections) {
                    if(restoredState[collectionName] && Array.isArray(restoredState[collectionName])) {
                        ...
                        restoredState[collectionName].forEach(item => {
                            // Garante que cada item tenha um ID
